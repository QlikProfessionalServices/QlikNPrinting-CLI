<#
.SYNOPSIS
	Creates an authenticated NPrinting session.
.DESCRIPTION
	Connect-NPrinting establishes the $script:NPEnv session variable used to
	authenticate subsequent requests made with Invoke-NPRequest.
.PARAMETER Prefix
	http or https prefix for the connection (default: https).
.PARAMETER Computer
	The NPrinting server to connect to (default: local computer). May also be a
	full URL (https://server:4993), in which case Prefix/Port are parsed from it.
.PARAMETER Port
	The port to connect on (default: 4993).
.PARAMETER Return
	Return the authentication token.
.PARAMETER Credentials
	Credential to authenticate the connection with. Required for NPrinting auth.
.PARAMETER TrustAllCerts
	Trust all TLS certificates (for self-signed NPrinting servers).
.PARAMETER AuthScheme
	Authentication type: ntlm (default) or the built-in NPrinting login.
.EXAMPLE
	Connect-NPrinting -Computer npserver -TrustAllCerts
.EXAMPLE
	Connect-NPrinting -Computer npserver -AuthScheme NPrinting -Credentials (Get-Credential)
#>
function Connect-NPrinting {
	...
	if ($AuthScheme -eq 'NPrinting' -and $null -eq $Credentials) {
		throw "NPrinting authentication requires -Credentials."
	}

	if ($TrustAllCerts.IsPresent) {
		Enable-NPTrustAllCerts
	}

	# A bare local computer name only works if the web engine runs locally.
	if ($Computer -eq $env:COMPUTERNAME) {
		$npService = Get-Service -Name 'QlikNPrintingWebEngine' -ErrorAction SilentlyContinue
		if ($null -eq $npService -or $npService.Status -ne 'Running') {
			throw "Local computer name used but the QlikNPrintingWebEngine service is not running locally. Specify -Computer explicitly."
		}
	}

	# Allow Computer to carry a full URL / port (e.g. https://server:4993).
	if ($Computer -match ':') {
	...
	if ($null -ne $Credentials) {
		$script:NPEnv.Credentials = $Credentials
	}

	$script:NPEnv.URLServerBase = "$($script:NPEnv.Prefix)://$($script:NPEnv.Computer):$($script:NPEnv.Port)"
	$script:NPEnv.URLServerAPI = "$($script:NPEnv.URLServerBase)/$($script:NPEnv.API)/$($script:NPEnv.APIVersion)"
	$script:NPEnv.URLServerNPE = "$($script:NPEnv.URLServerBase)/npe"

	$wrs = $script:NPEnv.WebRequestSession
	$wrs.UserAgent = 'Windows'
	$wrs.Cookies = New-Object System.Net.CookieContainer
	if ($null -ne $Credentials) {
	...
	if ($AuthScheme -eq 'NPrinting') {
		$loginUrl = $script:NPEnv.URLServerBase
	}
	else {
		$loginUrl = "$($script:NPEnv.URLServerAPI)/login/ntlm"
	}

	Write-Verbose $loginUrl
	...
	if ($AuthScheme -eq 'NPrinting') {
		# With NPrinting auth we first fetch the X-XSRF-Token (above), then post
		# the credentials to the login endpoint.
		$body = @{
		...
		$paramInvokeNPRequest = @{
			Path   = "$($script:NPEnv.URLServerBase)/login"
			Method = 'Post'
			Data   = $body
		}
		if ($PSBoundParameters.Debug.IsPresent) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUAB