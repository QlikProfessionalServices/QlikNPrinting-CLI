<#
.SYNOPSIS
	Gets the Tasks defined in NPrinting and caches them in $script:NPTasks.
.PARAMETER ID
	Id of a specific task to get.
.PARAMETER Name
	Name to filter the returned tasks on (supports the * wildcard).
.PARAMETER Executions
	Also retrieve the execution history for each task.
.PARAMETER Update
...
function Get-NPTasks {
	...
	$tasks = $script:NPTasks
	if ($PSBoundParameters.ContainsKey('Name')) {
		$tasks = $tasks | Where-Object { $_.name -like $Name }
	}

	if ($Executions.IsPresent) {
		$tasks | ForEach-Object {
			$executionPath = "tasks/$($_.id)/Executions"
			$taskExecutions = Invoke-NPRequest -Path $executionPath -Method Get
			Add-Member -InputObject $_ -MemberType NoteProperty -Name 'Executions' -Value $taskExecutions -Force
		}
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCbAzZxHtKitDo1
# 14wnrgKYmkxbUqgAK6bDHUx5rKcfp6CCGb0wggN5MIIC/qADAgECAhAcz51nzeIZ
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCDdvVyR
# TnOPGeS221BoQB0LoW6u80HtfdCp8CcBEoLBgzAKBggqhkjOPQQDAgRnMGUCMHIQ
# Ok1b0DMtG8nLe40/Qj6D/qxRIv/GR6372O5F0JmQJDvHNvxRb1HwlmqkSpGtOwIx
# AK3eT35KK/BrHqYjsxx65yGWn0UZGkMuPARQrJhym8YFKrSxEsynCiv8eUgm8fO9
# 2qGCA4QwggOABgkqhkiG9w0BCQYxggNxMIIDbQIBATBzMF4xCzAJBgNVBAYTAkJF
...
# HAYJKoZIhvcNAQkFMQ8XDTI2MDcwMTE0NDYwOFowKwYJKoZIhvcNAQk0MR4wHDAL
# BglghkgBZQMEAgKhDQYJKoZIhvcNAQEMBQAwPwYJKoZIhvcNAQkEMTIEMN81g+sE
# W6Otkbmm7oMEppPg4Av0Blmi8UdGTWVIO5ifk+brufPyrl5EinCzjSu0RDCBqAYL
...
# MDI1AhEAhHI/uDAN+6h1sztXzCY3gjANBgkqhkiG9w0BAQwFAASCAYBL60je56FW
# ReBDhGYWHJc6zexYtENsOSKUyLVZ1jEGjgSl4NKWv4qy9iofwAACPBQv/qB5wWcF
# jIU+y8y00Re5ZgiIZhjIen0fFBpmSe+sEDukgh7QcbgfNSy0z/roglteedzI7yBm
# TH5WihOwDAyr88/VNFjlt/PhyLRkM8s4jswH8faVaLQJ6szPclQYPkMxNsWdLonX
# 8rwDqaQfQKJq6Jg3vFNmAy2mm8DM7oQoSnVY1CM1aUOH43Oceez9G9dnrDMWH1YI
# mMQfm2mwFnvX1BKV6ia0QzFJKvLlUvP5J8Yy9peMS+TrCPCFPoQe4ybxR1q3lj6x
# abny8zpVbd3FXAo4FchN+ix/C2awc9JbL7KpKbdq0sGgvBJgPOu5Qry69INBb4dD
# cc4Cdbc/vuyaOt4DEyCvmM1CkD+LvElhO6hMgCoFWixyJKQ+0KUz6l7Z2EApRFlL
# GS+5jbEihbTIboRxJqwVgIwsTxzn8s/ZpTCLTDzY8LA8Eztu39/OkBQ=
