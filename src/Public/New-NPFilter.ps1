<#
.SYNOPSIS
	Creates a new NPrinting filter.
.DESCRIPTION
	POST /filters. Pass the filter body as a hashtable or object (the natural flow
	is to build one, or Get an existing filter, tweak it, and create a copy).
	Returns the new filter's id and location.
.PARAMETER InputObject
	The filter body (hashtable or PSObject), e.g.
	@{ name = 'Region EU'; appId = $appId; enabled = $true; fields = @(...) }.
.EXAMPLE
	New-NPFilter -InputObject @{ name = 'EU'; appId = $appId; enabled = $true }
#>
function New-NPFilter {
	...
	process {
		if ($PSCmdlet.ShouldProcess("filter '$($InputObject.name)'", 'Create')) {
			Invoke-NPRequest -Method Post -Path 'filters' -Data $InputObject
		}
	}
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDun7JFGu85Re1p
# GQFdueBc9TQEM6fMIE260hoU+Ur1VaCCGb0wggN5MIIC/qADAgECAhAcz51nzeIZ
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCAE966M
# YNMtIGn3xxRT5GnYHBSao6jcyrfLYIXr+/HeBDAKBggqhkjOPQQDAgRoMGYCMQC0
# PW8J8t7/SvDIWYdpbucZo7QNiR1kPbHTl4cVs6+Pi4oOjXPzUUXilAdJfxmh0KsC
# MQCyl3VkmadFGFOEtAZzMFzi2UzixToNePVI0KvvWOLGhxvEzmcHRLZ13qe+6vJV
# zB2hggOEMIIDgAYJKoZIhvcNAQkGMYIDcTCCA20CAQEwczBeMQswCQYDVQQGEwJC
...
# MBwGCSqGSIb3DQEJBTEPFw0yNjA3MDExNDQ2MTVaMCsGCSqGSIb3DQEJNDEeMBww
# CwYJYIZIAWUDBAICoQ0GCSqGSIb3DQEBDAUAMD8GCSqGSIb3DQEJBDEyBDDyDxHH
# yPfeG8NkKbdl8FYwd0ubZc3CsQI2PFtk3JNZ2UFu3MTjIYtGriOwMZFrbPUwgagG
...
# MjAyNQIRAIRyP7gwDfuodbM7V8wmN4IwDQYJKoZIhvcNAQEMBQAEggGAYx7MvJS/
# Ff6zhHnnvfpOaw6uMWIfyYFqA5bNC3n+hRpTvj+l5aljAJHQQcmlRcPvw11aP2YF
# /rJh8T8RfpBaFmNQTEyfM5psfvs/XaGLpaH2J0Tc4qrj6ebmiZC9yBbwqccuPIf2
# /ccLM9//sE1Cez1IHDNnlWhi2ipeAr/dWdbvq9efm4VRR6e1pYRUdhbXPqFF+JFk
# MZu9W0Pd0CM3zAwNu6MIbVaotlDccTtVWfKZ4mK6V0PD3HjF+JGUvOVg8FKlw742
# 4v48PQ79ZcOq74g0Z5k1gGwsbXYbWgxOMjk8G6yaccCDWtsu/KYTXbp7CMoFVhtO
# FZfz1JJxSYpJYGC/2ruYs2XLlQiOh6MRsEME5uL8veJcyyX6flPETc7nf2cqrigS
# OhbNJTTaaK617odKhHCLNkXdxZTCFdN9OrxNjz2kMvivuiQc0miPtONhlekuHmfz
# imX0gNyIaS8vtDEa67ZVa0f3HPzQ7EaLQJY66ybkmC/GzM196Hs+GCEY
