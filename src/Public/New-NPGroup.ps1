<#
.SYNOPSIS
	Creates a new NPrinting group.
.DESCRIPTION
	POST /groups. Provide a name, or a full body via -InputObject. Returns the new
	group's id and location.
.PARAMETER Name
	The group name.
.PARAMETER Enabled
	Whether the group is enabled (default: $true).
.PARAMETER InputObject
	A full group body (overrides -Name based construction).
.EXAMPLE
	New-NPGroup -Name 'Finance'
#>
function New-NPGroup {
	...
	process {
		$body = if ($PSCmdlet.ParameterSetName -eq 'ByObject') { $InputObject } else { @{ name = $Name; enabled = $Enabled } }
		$target = if ($PSCmdlet.ParameterSetName -eq 'ByObject') { "group '$($InputObject.name)'" } else { "group '$Name'" }
		if ($PSCmdlet.ShouldProcess($target, 'Create')) {
			Invoke-NPRequest -Method Post -Path 'groups' -Data $body
		}
	}
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAKzDP+pyXYDuaW
# pMIaZZcNk33Mi2dbxpVCqfjTz0GNF6CCGb0wggN5MIIC/qADAgECAhAcz51nzeIZ
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCBAplrb
# 0FcCVi9TTtlJHaVr0Lojns7Q44GXwlQhYPzlHDAKBggqhkjOPQQDAgRnMGUCMARS
# oGOd7Xxh7UoZ8zEtPS+24Gmhv2WJ2N3IaEpUUDP35fTnBzB+VzBdwGn/Ab/dJQIx
# ALJBsIodF2YVa6PBzvraRfTgjKGeHSYSUIxjI4ekstSVnkUMKEtqmRDdNxedxj7b
# 7KGCA4QwggOABgkqhkiG9w0BCQYxggNxMIIDbQIBATBzMF4xCzAJBgNVBAYTAkJF
...
# HAYJKoZIhvcNAQkFMQ8XDTI2MDcwMTE0NDYxNlowKwYJKoZIhvcNAQk0MR4wHDAL
# BglghkgBZQMEAgKhDQYJKoZIhvcNAQEMBQAwPwYJKoZIhvcNAQkEMTIEMN74YDKZ
# vwXJzIcGake4T91ltSlj6Pk/lkmSNAwA2tNeiRBnjpqsyYN0SYQkSI8UajCBqAYL
...
# MDI1AhEAhHI/uDAN+6h1sztXzCY3gjANBgkqhkiG9w0BAQwFAASCAYCA/Hp2Qv+x
# k9P3dQHsgdHW7J3tveMRms88gYRKjseViqXqJH2cr27G7T1LcFgV2WdjC7ha/xnr
# si+vxPaut3Xb354lTmlvl96oaZeeHwlAxk3+T/VCkvLKxn7WcDseHp0NafVW+DLm
# ekQL0/JWEeX//ETaxXDcxTYViCGnbfZvI6dCeGvLQPY6567P/abeerIjelu2CEYT
# mPcV5YYl+cM2isrqPV4BV4hVaICOKCi7ha1YC5uz1TR1I6vEtL+36fYkYCab/UjH
# fDLrooCaLzO4q3FJjyWunZk7JHasnrjOirkbI5bE7QakT9ULerbPICl0eJYuWaAX
# KG0/+0mYPPOi7xDYodVmF8yQKNDAbgvvqBvFGqIy6CIsEP4wtL9gtKO9aOP5JMuN
# fxBo7eqEYVhOwTiA17pfRWFnybEVymfC0gz41VpQ7ryFDVqIMlGIKopX7Fyn47hI
# EWBiVT3n4dW9NRoLxsJLgHWlSG0tH5pO65P9D0FuQMI7JtPwEWfwqBI=
